...
const spiralize = (n) => {
  ...
  const changeColumn = (rowIndex) => (column, columnIndex) => {
    const shouldReverse = rowIndex <= n / 2 - 1 + Math.sign(n % 4)
      && rowIndex - columnIndex === 1;
    const rowIndexMirror = (rowIndex < n / 2) ? rowIndex : n - 1 - rowIndex;
    const colIndexMirror = (columnIndex < n / 2) ? columnIndex : n - 1 - columnIndex;
    return (
      rowIndexMirror % 2 === 1 && rowIndexMirror <= colIndexMirror ||
        colIndexMirror % 2 === 1 && rowIndexMirror >= colIndexMirror
        ? (0 - shouldReverse) ** 2
        : 1 - shouldReverse
    );
  }

  const makeSpiral = (row, rowIndex) => row.map(changeColumn(rowIndex));
...
Dictionary.prototype.calculateChanges = function (term, word) {
  const dp = Array.from({ length: term.length + 1 }, (_, i) => i);

  for (let i = 1; i <= word.length; i++) {
    let prev = i;
    for (let j = 1; j <= term.length; j++) {
      const current = dp[j];
      dp[j] = word[i - 1] === term[j - 1] ? prev : Math.min(dp[j - 1], dp[j], prev) + 1;
      prev = current;
    }
  }

  return dp[term.length];
}
